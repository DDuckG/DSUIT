...
def _warmup_pipeline(det, seg_model, dep, scaler, enh, W, H, y_imgsz):
    ...
    depth_m = alpha / (np.maximum(depth_rel, 0.000001))
    _ = enh(depth_m, rgb0)
...
def process_video(src_path: str, out_path: str, cfg: dict):
    ...
    det = YOLODetector( ycfg["weights"], conf = ycfg["conf"], iou = ycfg["iou"], half = ycfg["half"], whitelist = ycfg.get("whitelist_classes", None), device = device, imgsz = tuple(ycfg.get("imgsz", (640, 384))))
    ...
    dep = DepthStreamVDA(encoder = dcfg["encoder"], checkpoint = dcfg["checkpoint"],input_size = dcfg["input_size"], device = device, fp32 = dcfg["fp32"])
    ...
    # Alerts 
    min_interval = float(cfg.get("audio", {}).get("min_interval_s", 2.0))
    scheduler = AlertScheduler(min_interval_s = min_interval)
    ...
    for packet in reader:
        ...
        if boxes.shape[0] > 0:
            boxes, scores, dists = prune_obs_slivers(boxes_xyxy = boxes, scores = scores, dists_m = dists, yolo_boxes = yolo_boxes_np, cfg = slcfg)

        # Obs track
        D = torch.from_numpy(depth_m_enh).to(device = device, dtype = torch.float32)
        Dl = torch.log(torch.clamp(D, min = 0.001))
        gx, gy = sobel_grad(Dl)
        ...
        roi_red = 0
        roi_yellow = 0
        for i in range(draw_boxes_arr.shape[0]):
            x1, y1, x2, y2 = draw_boxes_arr[i]
            d = float(draw_dists_arr[i]) if i < draw_dists_arr.shape[0] else float("nan")
            iw = max(0.0, min(x2, roi_x2) - max(x1, roi_x1))
            ih = max(0.0, min(y2, roi_y2) - max(y1, roi_y1))
            if iw <= 0.0 or ih <= 0.0:
                continue
            level = "none"
        ...
        scheduler.update(t_s, roi_level, debug_print = print)
        # print(f"[DBG] fid={frame_id} t={t_s:.2f}s det_yolo={yb.shape[0]} obs={len(obs_tracks)} ytrk={len(y_tracks)} roi={roi_level} (R={roi_red},Y={roi_yellow})")

        vis = draw_boxes(rgb, draw_boxes_arr, labels, draw_dists_arr, thr = (thr_low, thr_high), thickness = cfg["viz"]["thickness"], font_scale = cfg["viz"]["font_scale"])

        # Vẽ ROI để debug trực quan
        cv2.rectangle(vis, (roi_x1, roi_y1), (roi_x2, roi_y2), (128, 200, 255), 1, cv2.LINE_AA)

        hud = [f"OBS:epp={e_boxes.shape[0]} ppp={p_boxes.shape[0]} pole={v_boxes.shape[0]} out={len(obs_tracks)}  YOLO:{yb.shape[0]} trk={len(y_tracks)}  alpha={alpha:.3f}  roi={roi_level}"]
    ...
    alert_json = base + "_alerts.json"
    scheduler.save_json(alert_json)

    # require_frames = bool(cfg.get("io", {}).get("fail_on_zero_frames", False))
    # if written_frames == 0:
    #     msg = "[PIPE] wrote 0 frames."
    #     if require_frames:
    #         print(msg + " Raising RuntimeError due to io.fail_on_zero_frames=True")
    #         raise RuntimeError("No frames were processed/written. Check input video/codec.")
    #     else:
    #         try:
    #             shutil.copyfile(src_path, out_path)
    #             print(msg + f" Fallback copy source -> {out_path}")
    #         except Exception as e:
    #             print(msg + f" Fallback copy failed: {e}")
    #             raise
...
if __name__ == "__main__":
    ...
    os.makedirs(os.path.dirname(argument.out) or ".", exist_ok = True)
    process_video(argument.src, argument.out, config)
