...
def backproject_grid(W, H, fx, fy, cx, cy, stride = 1):
    ...
    fx_t = torch.tensor(fx, dtype = torch.float32)
    ...
    dirs = dirs / (torch.linalg.norm(dirs, dim = -1, keepdim = True) + 0.000000001)
    return dirs, None

def fit_plane_ls(points: torch.Tensor):
    N = points.shape[0]
    ones = torch.ones((N,1), dtype = points.dtype, device = points.device)
    A = torch.cat([points, ones], dim = 1)
    U, S, Vh = torch.linalg.svd(A, full_matrices = False)
    p = Vh[-1, :]
    n = p[ : 3]
    d = p[3]
    norm = torch.linalg.norm(n) + 0.000000001
...
def fit_plane_ransac(points: torch.Tensor, inlier_thr: float = 0.05, max_iter: int = 150):
    ...
    best_in = -1
    best_n = torch.tensor([0,1,0], dtype = torch.float32, device = points.device)
    best_d = torch.tensor(0.0, dtype = torch.float32, device = points.device)
    N = points.shape[0]
    ...
    for _ in range(int(max_iter)):
        ...
        if torch.linalg.norm(n) < 0.000001: 
            continue
        n = n / (torch.linalg.norm(n) + 0.000000001)
        d = -torch.dot(n, p1)
...
class GroundPlaneScaler:
    def __init__(self, W, H, fov_deg_h, principal_xy, cam_h = 1.35, sample_stride = 4, ema_beta = 0.9, use_sidewalk = True, use_terrain = True, method = "ransac", inlier_thr = 0.05, max_iter = 150):
    ...
    def estimate_scale(self, depth_rel: np.ndarray, seg: np.ndarray):
        ...
        if int(m.sum().item()) < 100:
            Hs, Ws = r.shape
            m2 = torch.zeros_like(m, dtype = torch.bool)
            m2[Hs // 2 : , :] = True
        ...
        if pts.shape[0] >= 3:
            ...
            n = torch.tensor([0,1,0], dtype = torch.float32, device = device)
            d = torch.tensor(0.0, dtype = torch.float32, device = device)
        ...
        self.n = self.n / (torch.linalg.norm(self.n) + 0.000000001)
    ...
    def height_from_plane(self, depth_m: np.ndarray, plane):
        ...
        if self._dirs_full is None or self._dirs_full.shape[ : 2] != (height, width):
            dirs_full, _ = backproject_grid(width, height, self.fx, self.fy, self.cx, self.cy, stride = 1)
