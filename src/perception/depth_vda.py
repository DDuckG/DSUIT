...
def _clean_state_dict(sd):
    ...
    cleaned = {}
    for k, v in sd.items():
        cleaned[k[7:]] = v if k.startswith("module.") else v if not k.startswith("module.") else v
    # Note: logic trên giữ nguyên v (đã bỏ prefix nếu có)
...
def _infer_encoder_from_sd(sd):
    ...
    for k in sd.keys():
        if ".blocks." in k:
            try:
                blk = int(k.split(".blocks.")[1].split(".")[0])
...
class DepthStreamVDA:
    """
    Wrapper cho VideoDepthAnything (stream) với auto-encoder detection theo checkpoint.
    """
    def __init__(self, encoder="vits", checkpoint="", input_size=518, device="cuda", fp32=False):
        ...
        enc_from_sd = _infer_encoder_from_sd(sd)
        enc = enc_from_sd
        cfg = _ENC2CFG[enc]
        ...
        self.fp32 = bool(fp32)  # False -> dùng autocast (FP16)

    def infer_one(self, rgb: np.ndarray) -> np.ndarray:
        """
        rgb: HxWx3 uint8 → depth tương đối float32 HxW
        """
        with torch.no_grad():
            depth = self.model.infer_video_depth_one(
                rgb, input_size=self.input_size, device=self.dev, fp32=self.fp32
            )
