...
class Track:
    __slots__=("bbox","score","clsid","id","age","time_since_update","hits","max_age")
    def __init__(self, bbox, score, clsid, tid, max_age):
    ...
    def predict(self):
        self.age+=1; self.time_since_update+=1; return self.bbox
    ...
    def dead(self): return self.time_since_update > self.max_age

class OCSort:
    def __init__(self, max_age=30, iou_thr=0.3, device=None):
    ...
    def update(self, det_xyxy, det_scores, det_clsids):
        ...
        det_xyxy = det_xyxy.to(self.device, dtype=torch.float32, non_blocking=True)
        det_scores= det_scores.to(self.device, dtype=torch.float32, non_blocking=True)
        det_clsids= det_clsids.to(self.device, dtype=torch.int64,   non_blocking=True)
        ...
        if T==0 and D>0:
            for i in range(D):
                tr=Track(det_xyxy[i], det_scores[i], det_clsids[i], self.next_id, self.max_age)
            ...
            ious = torch.where(same, ious, torch.zeros_like(ious))
            ...
            for di in range(D):
                if di not in used_d:
                    tr=Track(det_xyxy[di], det_scores[di], det_clsids[di], self.next_id, self.max_age)
                    self.next_id+=1; self.tracks.append(tr)
        ...
        for t in self.tracks:
            if t.time_since_update==0:
                out.append((t.id, t.bbox.detach().float().cpu().numpy(), t.score, t.clsid))
        return out
