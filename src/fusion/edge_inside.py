...
def _quantile_t(x, q):
    xv = x.reshape(-1)
    k = int(max(1, min(xv.numel(), round((float(q) * (xv.numel()-1)) + 1))))
    k = max(1, min(xv.numel(), k))
    val = torch.kthvalue(xv, k).values
    return val
...
def _ribbons(depth_t: torch.Tensor, box, r=3, side="right"):
    ...
    H,W = depth_t.shape
    x1 = _clamp(x1, 0, W-2); x2 = _clamp(x2, x1+1, W-1)
    y1 = _clamp(y1, 0, H-2); y2 = _clamp(y2, y1+1, H-1)
    r = int(max(1, r))

    if side == "right":
        Rin  = depth_t[y1:y2, _clamp(x2-r,0,W-1):x2]
        Rout = depth_t[y1:y2, _clamp(x2,0,W-1):_clamp(x2+r,0,W)]
    elif side == "left":
        Rin  = depth_t[y1:y2, _clamp(x1,0,W-1):_clamp(x1+r,0,W)]
        Rout = depth_t[y1:y2, _clamp(x1-r,0,W-1):_clamp(x1,0,W)]
    elif side == "top":
        Rin  = depth_t[_clamp(y1,0,H-1):_clamp(y1+r,0,H), x1:x2]
        Rout = depth_t[_clamp(y1-r,0,H-1):_clamp(y1,0,H), x1:x2]
    else:  # bottom
        Rin  = depth_t[_clamp(y2-r,0,H-1):y2, x1:x2]
        Rout = depth_t[_clamp(y2,0,H-1):_clamp(y2+r,0,H), x1:x2]
    return Rin, Rout
...
def snap_inside_box(box_np: np.ndarray, depth_t: torch.Tensor, cfg: dict):
    """
    Snap lại 3 cạnh (left/right/top) để đảm bảo mép nằm *trong* box
    và có chênh lệch độ sâu đúng phía (gần hơn ở trong).
    """
    b = np.asarray(box_np, dtype=np.float32).reshape(4)
    ...
    # RIGHT
    candidates = torch.arange(max(0, x2-sdx), min(W-1, x2+sdx)+1, device=depth_t.device)
    ...
    # LEFT
    candidates = torch.arange(max(0, x1-sdx), min(W-1, x1+sdx)+1, device=depth_t.device)
    if candidates.numel() > 0:
        best = x1; best_dz = -1.0
        for x in candidates.tolist():
    ...
    # TOP
    candidates = torch.arange(max(0, y1-sdy), min(H-1, y1+sdy)+1, device=depth_t.device)
    if candidates.numel() > 0:
        best = y1; best_dz = -1.0
...
@torch.no_grad()
def edge_pair_ok(depth_t: torch.Tensor, box_np: np.ndarray, cfg: dict):
    ribbon_px = int(cfg.get("ribbon_px", 3))
    jmin = float(cfg.get("jump_min_m", 0.25))
    vinm = float(cfg.get("var_in_max", 0.08))
    fmin = float(cfg.get("fill_min", 0.55))
    ...
    count = int(okR)+int(okL)+int(okT)+int(okB)
    return bool(count >= 2)
