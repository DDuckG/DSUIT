...
@torch.no_grad()
def detect_planar_obstacles(depth_m_enh: np.ndarray, sigma_m: np.ndarray, seg: np.ndarray, height_m: np.ndarray, cfg: dict, fx: float, fy: float):
    dev = get_device()
    ...
    dists = []
    scores = []
    area_k = float(cfg.get("area_min_m2_k", 0.0004))
