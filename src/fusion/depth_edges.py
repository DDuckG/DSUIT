...
def _edge_maps_from_depth_tiled(depth_m: torch.Tensor, cfg: dict):
    ...
    for i in range(rows):
        for j in range(cols):
            y1 = i*rh; y2 = (i+1)*rh if i < rows-1 else H
            x1 = j*cw; x2 = (j+1)*cw if j < cols-1 else W
...
def _cut_bottom_by_height(boxes_np: np.ndarray, height_m_t: torch.Tensor,
                          h_free=0.05, band_px=12, min_drop=0.10):
    if boxes_np is None or len(boxes_np) == 0:
        return boxes_np
    out=[]
    ...
    for x1,y1,x2,y2 in boxes_np.astype(int):
        ...
        # median thay vì nanmedian để tương thích torch cũ
...
def _two_edge_ok(box, gx: torch.Tensor, gy: torch.Tensor, k:int=3, vthr:float=0.0, hthr:float=0.0):
    ...
    y1=max(0,min(H-2,y1)); y2=max(y1+1,min(H-1,y2))
    ks = int(max(1,k))
    vband = slice(y1,y2), slice(max(0,x1-ks), min(W-1,x1+ks+1)), slice(max(0,x2-ks), min(W-1,x2+ks+1))
    hband = slice(max(0,y1-ks), min(H-1,y1+ks+1)), slice(x1,x2)
...
def detect_obstacles_by_depth_edges(depth_m_enh: np.ndarray,
    ...
    boxes = []; dists = []; scores=[]; edgeoks=[]
    H, W = D.shape
    area_k = float(cfg.get("area_min_m2_k", 3.0e-4))
    for i in range(len(y1_h)):
        ...
        b = boxes_np[0]
        # 2-edge rule
        vthr = float(cfg.get("twoedge_v_min", 0.0025))
        ...
        boxes.append(b); dists.append(z); scores.append(sc); edgeoks.append(True)
