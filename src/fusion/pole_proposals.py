...
def detect_poles_from_depth(depth_m: np.ndarray, cfg: dict):
    """
    Đề xuất cột/biển mảnh dọc từ gradient theo trục dọc.
    """
    ...
    gx, gy = sobel_grad(Dl)
    gy = torch.abs(gy)
    col_energy = gy.sum(dim=0)  # [W]
    thr = torch.quantile(col_energy, 0.90)
    mask = (gy >= (thr / (depth_m.shape[0]))).to(torch.uint8)  # normalize thô theo H
    ...
    for i in range(len(y1_h)):
        ...
        scores.append(min(1.0, h / float(H)))
        dists.append(z)
