...
class ObsTracker:
    def __init__(self, iou_thr: float = 0.5, max_age: int = 0, z_max: float = 15.0, edge_fail_kill: int = 2, staging: dict | None = None, lock: dict | None = None):
    ...
    def update(self, det_boxes: np.ndarray, det_scores: np.ndarray, det_dists: np.ndarray, refine_fn: Callable[[np.ndarray], np.ndarray], image_wh: Tuple[int,int], edge_ok: np.ndarray, yolo_boxes: np.ndarray):
        width, height = image_wh
        detected = det_boxes.shape[0]
        T = len(self.tracks)
        ...
        if detected > 0:
            ...
            det_boxes = det_boxes[zmask]
            det_scores = det_scores[zmask]
            det_dists = det_dists[zmask]
            edge_ok = edge_ok[zmask]
            detected = det_boxes.shape[0]

        if T > 0 and detected > 0:
            track_boxes = np.stack([track.bbox for track in self.tracks], axis = 0)
            M = iou_matrix(track_boxes, det_boxes)
            matches = []
            used_t = set()
            used_d = set()
            while True:
                ti, di = divmod(np.argmax(M), detected if detected > 0 else 1)
                ...
                M[ti, : ] = -1.0
                M[:, di] = -1.0
                if len(used_t) == T or len(used_d) == detected:
                    break

            for (ti, di) in matches:
                tr = self.tracks[ti]
                b = refine_fn(det_boxes[di])
                cx = 0.5 * (tr.bbox[0] + tr.bbox[2])
                cy = 0.5 * (tr.bbox[1] + tr.bbox[3])
                w0 = tr.bbox[2] - tr.bbox[0]
                h0 = tr.bbox[3] - tr.bbox[1]
                s = tr.z / max(0.000001, float(det_dists[di]))
                ...
                tr.bbox = (self.lock_beta * tr.bbox + (1.0-self.lock_beta)*b_locked).astype(np.float32)     # EMA
            ...
            for di in range(detected):      # unmatched detections → staging
                if di in used_d:
                    continue
                self._add_or_update_pending(det_boxes[di], det_scores[di], det_dists[di], edge_ok[di])
        elif detected>0:
            for di in range(detected):
                self._add_or_update_pending(det_boxes[di], det_scores[di], det_dists[di], edge_ok[di])

        self._staging_step(yolo_boxes, width, height, refine_fn)
        self._age_and_prune(width,height)
    ...
    def _age_and_prune(self, W, H):
        ...
        for track in self.tracks:
            ...
            frac = (in_w * in_h) / max(0.000001, (x2 - x1) * (y2 - y1))
            if track.time_since_update > self.max_age:
                continue
            if track.edge_fail >= self.edge_fail_kill:
                continue
            if track.z > self.z_max:
                continue
            if frac < 0.15:
                continue
            alive.append(track)
    ...
    def _dump(self):
        out = []
        for track in self.tracks:
            out.append((track.id, track.bbox.copy(), track.score, -1, float(track.z), track.time_since_update > 0))
        return out
