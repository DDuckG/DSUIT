...
def _clean_state_dict(sd):
    if isinstance(sd, dict) and "state_dict" in sd:
        sd = sd["state_dict"]
    return {(vat1[7: ] if vat1.startswith("module.") else vat1): vat2 for vat1,vat2 in sd.items()}

def _infer_encoder_from_sd(sd):
    embed_dim = None
    for vat1,vat2 in sd.items():
        if vat1.endswith("pos_embed") and isinstance(vat2, torch.Tensor) and vat2.dim() == 3:
            embed_dim = int(vat2.shape[-1]) 
            break
    if embed_dim in _EMBED2ENC: 
        return _EMBED2ENC[embed_dim]
    max_block = -1
    for vat1 in sd.keys():
        if ".blocks." in vat1:
            try: 
                max_block = max(max_block,int(vat1.split(".blocks.")[1].split(".")[0]))
            except: 
                pass
    if max_block >= 23: 
        return "vitl"
...
class DepthStreamVDA:
    def __init__(self, encoder = "vits", checkpoint = "", input_size = 518, device = "cuda", fp32 = False):
        ...
        cfg = _ENC2CFG[enc]
        self.model = VideoDepthAnything(encoder = enc, features = cfg["features"], out_channels = cfg["out_channels"],num_frames = 32, pe = "ape").to(dev).eval()
    ...
    def infer_one(self, rgb: np.ndarray) -> np.ndarray:
        depth = self.model.infer_video_depth_one(rgb, input_size = self.input_size, device = str(self.dev), fp32 = self.fp32)
